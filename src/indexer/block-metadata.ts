...
export async function handleBlockMetadata() {
  try {
    ...
    const lastBlockNumberIndexed = (await database(TABLE_NAME).max('blockNumber').first())?.blockNumber ?? 100 // TODO replace default with May 30 block
    await database.transaction(async (trx) => {
      await asyncPool(50, _.range(lastBlockNumberIndexed + 1, currentBlockNumber + 1), async (blockNumber) => {
        const blockTimestamp = (await provider.getBlock(blockNumber))?.timestamp
        if (!blockTimestamp) {
          throw new Error(`Could not get block timestamp for block ${blockNumber}`)
        }
